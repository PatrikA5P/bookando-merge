 ...
 * - Multiple absence types (vacation, sick, training, unpaid)
 */
class VacationRequestService
{
    ...
    // Absence types
    public const ABSENCE_TYPE_VACATION = 'vacation';
    public const ABSENCE_TYPE_SICK = 'sick';
    public const ABSENCE_TYPE_TRAINING = 'training';
    public const ABSENCE_TYPE_UNPAID = 'unpaid';

    /**
     * Get absence requests for display.
     *
    ...
    public static function getRequests(array $filters = []): array
    {
        ...
        if (!empty($filters['absence_type'])) {
            $where[] = 'd.absence_type = %s';
            $params[] = $filters['absence_type'];
        }
     ...
     *
     * @param int $userId Employee requesting absence
    ...
    public static function createRequest(int $userId, array $data): array
    {
        ...
        $absenceType = $data['absence_type'] ?? self::ABSENCE_TYPE_VACATION;
        if (!in_array($absenceType, [
            self::ABSENCE_TYPE_VACATION,
            self::ABSENCE_TYPE_SICK,
            self::ABSENCE_TYPE_TRAINING,
            self::ABSENCE_TYPE_UNPAID
        ], true)) {
            $absenceType = self::ABSENCE_TYPE_VACATION;
        }

        // Check for overlapping requests
        $overlap = self::checkOverlap($userId, $startDate->format('Y-m-d'), $endDate->format('Y-m-d'));
        if ($overlap) {
            throw new \RuntimeException('Overlapping absence request exists');
        }

        // Sick leave is auto-approved (no approval workflow needed)
        $initialStatus = $absenceType === self::ABSENCE_TYPE_SICK ? self::STATUS_APPROVED : self::STATUS_PENDING;

        // Set default name based on type
        $defaultName = match ($absenceType) {
            self::ABSENCE_TYPE_SICK => 'Krankmeldung',
            self::ABSENCE_TYPE_TRAINING => 'Fortbildung',
            self::ABSENCE_TYPE_UNPAID => 'Unbezahlter Urlaub',
            default => 'Urlaub',
        };
        ...
        if ($absenceType === self::ABSENCE_TYPE_SICK) {
            $requestData['reviewed_by'] = $userId;
            $requestData['reviewed_at'] = wp_date('Y-m-d H:i:s');
        }

        $wpdb->insert(
            $wpdb->prefix . 'bookando_employees_days_off',
            $requestData,
            ['%d', '%s', '%s', '%s', '%s', '%d', '%s', '%s', '%d', '%s', '%d', '%s']
        );
    ...
    private static function formatRequest(array $row): array
    {
        return [
            ...
            'absence_type' => $row['absence_type'] ?? self::ABSENCE_TYPE_VACATION,
