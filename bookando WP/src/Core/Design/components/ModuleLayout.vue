  ...
  <div class="flex flex-col min-h-full bg-slate-50/50">
          ...
          <div v-if="tabs && tabs.length > 0" class="flex overflow-x-auto no-scrollbar gap-2 pb-1">
            <button
              ...
              :class="[
                ...
                activeTab === tab.id ? 'bg-white text-accent-700 shadow-sm' : 'bg-white/10 text-white hover:bg-white/20'
              ]"
            >
        ...
        <div class="px-4 pb-3 flex gap-2">
          ...
          <button
            ...
            :class="[
              ...
              isFilterOpen ? 'bg-white text-accent-700' : 'bg-white/10 text-white hover:bg-white/20'
            ]"
          >
    ...
    <div class="hidden lg:flex min-h-full p-6 gap-6 items-start">
      ...
      <aside v-if="tabs && tabs.length > 0" class="w-72 flex-shrink-0 flex flex-col gap-6 sticky top-6 self-start z-40">
        <!-- Hero Section -->
        <div :class="['relative overflow-hidden shrink-0 text-white p-6 shadow-lg rounded-xl', heroGradient]">
          ...
          <div class="absolute right-0 top-0 opacity-10 transform translate-x-2 -translate-y-1/2 pointer-events-none">
          ...
          <div class="sticky top-0 z-30 bg-slate-50 pt-6 pb-6 -mt-6">
            <div class="flex gap-6 items-stretch">
              ...
              <div class="w-72 shrink-0 z-20 relative">
                <div :class="['relative overflow-hidden text-white p-6 rounded-xl shadow-lg h-full flex flex-col justify-center', heroGradient]">
                  <div class="relative z-10">
                    <div class="flex items-center gap-3 mb-2">
                      <component :is="heroIcon" class="w-6 h-6 text-white/80" />
                      <h2 class="font-bold text-xl text-white">{{ heroTitle }}</h2>
                    </div>
                    <p class="text-xs max-w-2xl text-white/70">{{ heroDescription }}</p>
                  </div>
                  <div class="absolute right-0 top-0 opacity-10 transform translate-x-2 -translate-y-2 pointer-events-none">
                    <component :is="heroIcon" :size="96" />
                  </div>
                </div>
              </div>
