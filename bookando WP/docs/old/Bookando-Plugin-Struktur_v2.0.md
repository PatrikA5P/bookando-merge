# 📦 Bookando – Plugin- & SaaS-Struktur  (Technische Dokumentation v 2.1)
...
└── src/
    ...
    └── modules/                      # Alle Module, klein geschrieben!
        └── <slug>/                   # z. B. "customers"
            ...
            ├── Api/
            │   └── Api.php                 # register_routes(): RestDispatcher::register('<slug>', RestHandler::class)
            ├── Templates/
...
| **RestDispatcher** | `/wp-json/bookando/v1/<slug>/<type>[/{subkey}]` | Permission‑Callback; ruft `<Module>\RestHandler::<type>()` |
| **WebhookDispatcher** | `/wp-json/bookando/v1/webhook/<type>` | Token + Signatur (+ Replay‑Schutz) |

**Permission‑Callback (neu, zentral nutzbar):**
...
function bookando_allow(string $slug, ?string $feature = null): bool {
  return Gate::check()
    && current_user_can("manage_bookando_{$slug}")
    && ($feature ? LicenseManager::isFeatureEnabled($feature)
                 : LicenseManager::isModuleAllowed($slug));
}
```
**Beispiel‑Registrierung im Modul:**

```php
register_rest_route('bookando/v1', '/customers/list', [
  'methods'  => 'GET',
  'callback' => [ \Bookando\Modules\customers\RestHandler::class, 'list' ],
  'permission_callback' => fn() => bookando_allow('customers')
]);
```
