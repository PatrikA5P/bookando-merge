...
const AppointmentsModule: React.FC = () => {
  const { t, services, pricingRules, appointments, addAppointment, updateAppointment, deleteAppointment, customers } = useApp();
  ...
  const [currentDate, setCurrentDate] = useState<Date>(() => {
    if (typeof window !== 'undefined') {
      const storedDate = localStorage.getItem('appointmentsLastDate');
      if (storedDate) {
  ...
  const filteredAppointments = useMemo(() => {
    return appointments.filter(apt => {
      ...
      const customerMatch = filters.customerId === '' || apt.customerId === filters.customerId;
      ...
      return searchMatch && empMatch && customerMatch && catMatch && statusMatch;
    });
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('appointmentsLastDate', currentDate.toISOString());
    }
  }, [currentDate]);
  ...
  return (
    <div className="h-full flex flex-col space-y-4 animate-fadeIn">
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col gap-4">
          ...
          {isFilterExpanded && (
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 pt-2 animate-slideDown">
                  <div>
