...
const SettingsModule: React.FC = () => {
  ...
  const moduleDesign = getModuleDesign('settings');

  const tabs = [
    { id: 'general', icon: Globe, label: t('general') },
    { id: 'company', icon: Building, label: t('company_details') },
    { id: 'integrations', icon: LinkIcon, label: t('integrations') },
    { id: 'license', icon: Key, label: t('license_plan') },
    { id: 'modules', icon: ToggleLeft, label: t('modules') },
    { id: 'permissions', icon: Lock, label: t('roles_permissions') },
  ];
...
const SettingsNavItem: React.FC<{ active: boolean; onClick: () => void; icon: any; label: string }> = ({ active, onClick, icon: Icon, label }) => (
   <button
      ...
      className={`
         ...
         ${active ? 'bg-slate-100 text-slate-900 font-bold shadow-sm' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'}
      `}
   >
      <Icon size={18} />
      {label}
