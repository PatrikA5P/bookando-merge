...
import DesignTemplates from './modules/DesignTemplates';
import DesignFrontendModule from './modules/DesignFrontend';
...
const AppContent: React.FC = () => {
  ...
  const navItems: NavItem[] = [
    ...
    { name: ModuleName.DESIGN_SYSTEM, icon: Palette },
    { name: ModuleName.DESIGN_FRONTEND, icon: Layout }, // New Module
  ...
  const renderModule = () => {
    switch (activeModule) {
      ...
      case ModuleName.TOOLS:
        return <ToolsModule />;
      case ModuleName.DESIGN_SYSTEM:
        return <DesignTemplates />;
      case ModuleName.DESIGN_FRONTEND:
        return <DesignFrontendModule />;
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen bg-slate-50 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative z-0 transition-all duration-300">
        {/* Hide default header for Design modules as they have their own headers */}
        {activeModule !== ModuleName.DESIGN_SYSTEM && activeModule !== ModuleName.DESIGN_FRONTEND && <Header title={activeModule} />}
        
        <main className={`flex-1 overflow-y-auto scroll-smooth ${activeModule === ModuleName.DESIGN_SYSTEM || activeModule === ModuleName.DESIGN_FRONTEND ? 'p-0' : 'p-4 md:p-8'}`}>
          <div className={`${activeModule === ModuleName.DESIGN_SYSTEM || activeModule === ModuleName.DESIGN_FRONTEND ? 'h-full' : 'max-w-7xl mx-auto'} animate-fadeIn`}>
             {renderModule()}
          </div>
        </main>
      </div>
